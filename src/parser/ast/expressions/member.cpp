...
Value MemberExpression::evaluate(Context& ctx) {
    ...
    Value computed_key;
    if (computed_) {
        computed_key = property_->evaluate(ctx);
        if (ctx.has_exception()) return Value();
        prop_name = to_js_property_key(ctx, computed_key);
        if (ctx.has_exception()) return Value();
    } else {
        if (property_->get_type() == ASTNode::Type::IDENTIFIER) {
            Identifier* prop = static_cast<Identifier*>(property_.get());
    ...
    if (object_value.is_string()) {
        ...
        if (computed_) {
            // The key already ran once above; a second evaluate would repeat its
            // side effects, and only the Value (not prop_name) keeps symbol identity.
            const Value& prop_value = computed_key;
