...
std::string Value::to_property_key_strict(Context& ctx) const {
    ...
    Object* obj = is_function() ? static_cast<Object*>(as_function()) : as_object();
    Value key = obj->to_primitive(ctx, "string");
    if (ctx.has_exception()) return "";
    // A Symbol that came back from the conversion IS the key; only anything
    // else is stringified. Reading it as a string turned a symbol-keyed
    // property into one named after the symbol's description.
