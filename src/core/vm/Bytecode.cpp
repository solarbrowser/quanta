...
const OpInfo& op_info(Op op) {
    static const OpInfo table[] = {
        ...
        {"ToNumber", 0, '-'}, {"ToNumeric", 0, '-'}, {"Inc", 0, '-'}, {"ToTemplateString", 0, '-'},
        {"BuildTemplateString", 2, 'S'},
        {"ToPropertyKey", 0, '-'}, {"CheckObjectCoercible", 0, '-'}, {"Dec", 0, '-'},
        ...
        {"ForInKeyPresent", 3, 'J'},
        {"LdaArgLength", 0, '-'}, {"LdaArgAt", 0, '-'}, {"LdaFrameArgsMarker", 0, '-'}
    };
...
std::string disassemble_chunk(const BytecodeChunk& chunk, const std::string& name) {
    ...
    while (pc < chunk.code.size()) {
        ...
        switch (info.kind) {
            ...
            // Wide counterpart of 'z' (CreateClosureWide/DeclareFunctionWide):
            // same single-index shape, k32 instead of k16.
            case 'U': case 'V': {
                uint32_t idx = static_cast<uint32_t>(chunk.code[operand_pc]) |
                               (static_cast<uint32_t>(chunk.code[operand_pc + 1]) << 8) |
