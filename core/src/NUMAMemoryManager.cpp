...
bool NUMATopology::detect_numa_topology() {
    ...
    if (nodes_.empty()) {
        std::cout << "âš ï¸  No NUMA nodes detected, using single-node fallback" << std::endl;
        numa_available_ = false;
        return false;
    }
...
void NUMATopology::detect_windows_numa() {
    ...
    if (!GetNumaHighestNodeNumber(&highest_node)) {
        std::cout << "âš ï¸  GetNumaHighestNodeNumber failed" << std::endl;
        return;
    }
...
void NUMATopology::detect_linux_numa() {
    if (numa_available() == -1) {
        std::cout << "âš ï¸  NUMA not available on this system" << std::endl;
        numa_available_ = false;
        return;
    }
...
NUMAAllocator::NUMAAllocator() 
    ...
    node_mutexes_.resize(node_count);
...
bool NUMAMemoryManager::initialize() {
    ...
    if (!topology.detect_numa_topology()) {
        std::cout << "âš ï¸  NUMA not available, using fallback mode" << std::endl;
    }
