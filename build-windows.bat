...
REM Compiler flags
set "CXXFLAGS=-std=c++20 -Wall -O3 -march=native -mtune=native -DQUANTA_VERSION=\"0.9.0.71826\" -DPROMISE_STABILITY_FIXED -DNATIVE_BUILD -DUTF8PROC_STATIC -funroll-loops -finline-functions -fvectorize -fslp-vectorize -msse4.2 -mavx -mavx2 -fomit-frame-pointer -fstrict-aliasing -fstrict-enums -flto=thin"
...
REM Compile builtin modules
echo [1/4] Compiling builtin modules...
echo [%time%] === BUILTINS === >> "%LOG_FILE%"
for /r src\core\engine\builtins %%f in (*.cpp) do (
    set /a COMPILED_FILES+=1
    echo   [!COMPILED_FILES!/%TOTAL_FILES%] %%~nf.cpp
    clang++ %CXXFLAGS% %INCLUDES% -c %%f -o build\obj\core\engine\builtins\%%~nf.o 2>> "%ERROR_LOG%"
    if !ERRORLEVEL! NEQ 0 (
        echo   [FAILED] %%f
        echo [%time%] ERROR compiling %%f >> "%LOG_FILE%"
        set /a FAILED_FILES+=1
